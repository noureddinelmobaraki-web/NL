...
// XML-escape helper for safe titles/captions
...
function buildSongBlock(song) {
  const safeName = xmlEscape(song.name);
  return `  <url>
    ...
    <image:image>
      ...
      <image:title>${safeName} — NL</image:title>
      <image:caption>Listen to ${safeName} by NL on NRADIO</image:caption>
    </image:image>
  </url>`;
}

function generateSitemap() {
  const songBlocks = SONGS_DATA.map(buildSongBlock).join('\n\n');
...
<urlset
  ...
  <!-- ===========================================================
       ...
       Generated: ${TODAY}
       Total URLs: ${1 + SONGS_DATA.length}
       =========================================================== -->

  <!-- === MAIN PAGE ======================================= -->
${buildHomepageBlock()}

  <!-- === SONG SHARE PAGES (${SONGS_DATA.length}) ======================= -->
${songBlocks}
...
`;
  ...
  console.log(`✅ Updated public/sitemap.xml (${1 + SONGS_DATA.length} URLs)`);
  ...
  console.log(`✅ Generated dist/sitemap.xml (${1 + SONGS_DATA.length} URLs)`);
}
