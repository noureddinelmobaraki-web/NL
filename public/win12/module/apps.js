...
let apps = {
    ...
    imgviewer: {
        ...
        init: () => {},
        open: (url, name) => {
            // revoke previous blob before opening new image (memory leak fix)
            if (apps.imgviewer._blobUrl && !apps.imgviewer._isRemote) {
                try { URL.revokeObjectURL(apps.imgviewer._blobUrl); } catch (_) {}
            }
            openapp('imgviewer');
            apps.imgviewer._blobUrl  = url;
            apps.imgviewer._isRemote = /^https?:\/\//i.test(url);
            apps.imgviewer._scale = 1;
            apps.imgviewer._rotate = 0;
            apps.imgviewer._applyTransform();
            $('#win-imgviewer .preview-img').attr('src', url).attr('referrerpolicy', 'no-referrer');
        ...
        close: () => {
            ...
            apps.imgviewer._isRemote = false;
            hidewin('imgviewer');
        },
        zoomIn: () => {
    ...
    mediaplayer: {
        ...
        init: () => {},
        // type: 'video' | 'audio'
        // url:  blob: URL  OR  http(s) URL  OR  hls m3u8 URL (auto-detected)
        open: (url, name, type) => {
            // revoke previous blob if any
            if (apps.mediaplayer._blobUrl && !apps.mediaplayer._isRemote) {
                try { URL.revokeObjectURL(apps.mediaplayer._blobUrl); } catch (_) {}
            }
            openapp('mediaplayer');
            apps.mediaplayer._blobUrl  = url;
            apps.mediaplayer._isRemote = /^https?:\/\//i.test(url);
            $('.window.mediaplayer>.titbar>p').text(name || lang('媒体播放器', 'mediaplayer.name'));

            var $video = $('#mediaplayer-video');
            var $audio = $('#mediaplayer-audio');
            var elV = $video[0], elA = $audio[0];

            // detect HLS stream
            var isHls = /\.m3u8(\?|$)/i.test(url) || /\/manifest/i.test(url) || /smil:/i.test(url);
            var targetEl = (type === 'video') ? elV : elA;
            var $target  = (type === 'video') ? $video : $audio;
            var $other   = (type === 'video') ? $audio : $video;
            var otherEl  = (type === 'video') ? elA : elV;

            $other.hide();
            try { otherEl.pause(); otherEl.removeAttribute('src'); otherEl.load(); } catch (_) {}

            // destroy previous hls instance if any
            if (apps.mediaplayer._hls) {
                try { apps.mediaplayer._hls.destroy(); } catch (_) {}
                apps.mediaplayer._hls = null;
            }

            $('.window.mediaplayer>.titbar>img').attr('src',
                type === 'video' ? 'icon/files/vidio.png' : 'icon/files/music.png');

            if (isHls && window.Hls && window.Hls.isSupported() && type === 'video') {
                var hls = new window.Hls();
                apps.mediaplayer._hls = hls;
                hls.loadSource(url);
                hls.attachMedia(targetEl);
                $target.show();
            } else {
                $target.attr('src', url).show();
                try { targetEl.load(); } catch (_) {}
            }

            // attempt autoplay (silently fails without user gesture; controls remain visible)
            var p = targetEl.play && targetEl.play();
            if (p && typeof p.catch === 'function') p.catch(function () { /* needs user gesture */ });
        },
        close: () => {
            try { $('#mediaplayer-video')[0].pause(); } catch (_) {}
            try { $('#mediaplayer-audio')[0].pause(); } catch (_) {}
            if (apps.mediaplayer._hls) {
                try { apps.mediaplayer._hls.destroy(); } catch (_) {}
                apps.mediaplayer._hls = null;
            }
            ...
            apps.mediaplayer._isRemote = false;
            hidewin('mediaplayer');
        }
    },
    pdfviewer: {
