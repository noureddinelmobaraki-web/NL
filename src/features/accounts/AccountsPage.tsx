  ...
  return createPortal(
    <div className="nl-accounts" dir="rtl">
      <header className="nl-accounts__bar">
        <div className="nl-accounts__title"><Users size={20} /> الحسابات</div>
        <label className="nl-accounts__search">
          <Search size={16} />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث عن مستخدم..." />
        </label>
        <button type="button" className="nl-accounts__close" onClick={onClose} aria-label="إغلاق"><X size={20} /></button>
      </header>

      {error && <p className="nl-accounts__error">{error}</p>}
      {loading ? (
        <p className="nl-accounts__hint">جارٍ التحميل…</p>
      ) : rows.length === 0 ? (
        <p className="nl-accounts__hint">لا توجد حسابات مطابقة.</p>
      ) : (
        <div className="nl-accounts__grid">
          {rows.map((u) => (
            <button type="button" key={u.id} className="nl-acc-card nl-acct-card" onClick={() => setOpenId(u.id)}>
              ...
              <span className="nl-acc-card__name">{u.display_name || 'مستخدم'}</span>
