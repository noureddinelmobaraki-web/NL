...
/* Animated line-drawing sketch (Uiverse.io by SelfMadeSystem), scoped to
   .nld-window-sketch so it cannot leak. Keyframes namespaced nld-sketch-*. */
const SKETCH_CSS = `
.nld-window-sketch {
  position: absolute;
  inset: 0;
...
@keyframes nld-sketch-offset {
  0% { stroke-dashoffset: 365; }
  100% { stroke-dashoffset: 5; }
}
@media (prefers-reduced-motion: reduce) {
  .nld-window-sketch path { animation: none; stroke-dasharray: none; }
}
`;

/**
 * Small square glass "window" on Home. Instead of cross-fading through video
 * first-frames, it now shows a coded, self-drawing sketch (pure SVG + CSS).
 * It still hangs from the elastic rope as a real interactive pendulum and opens
 * the fullscreen gallery on click / Enter / Space. Expensive parts stay lazy
 * and viewport-gated by the hooks below.
 */
export const DrawingsWindow = ({ onOpen }: DrawingsWindowProps) => {
  ...
  // media latch: mount the sketch + glass filter on first appearance, then keep
